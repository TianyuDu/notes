...
\begin{document}
	...
	\begin{definition}
		...
		\end{align}
		For any measruable $f: X \to [-\infty, \infty]$, let
		\begin{align}
			f^+(x) &= \max\{f(x), 0\} \\
			f^-(x) &= - \min\{f(x), 0\}
		\end{align}
		So that $f = f^+ - f^-$, where $f^+, f^-: X \to [0, \infty]$ are measurable. Define $\int f\ d\mu = \int f^+\ d\mu - \int f^-\ d\mu$ provided that at least one term on the right is finite, otherwise, $\int f\ d\mu$ is undefined.
	\end{definition}
	...
	\begin{proposition}
		Let $f: X \to [0, \infty]$ be a \ul{measurable} function. Then there exists an \ul{increasing} sequence of \textbf{non-negative simple} functions $f_n$ such that $f_n \leq f_{n+1}$ and 
		\begin{align}
	...
	\begin{proposition}
		...
		\begin{proof}
			...
			\begin{align}
				...
				\end{cases}
			\end{align}
			By construction, $g_n \leq f_n$ and $\int g_n\ d\mu \leq \int f_n\ d\mu$.
			\begin{align}
				\lim \int f_n\ d\mu &\geq \lim g_n\ d\mu \\
				&= \lim \sum_{i=1}^K (1-\varepsilon) a_i \mu(A_{n,i}) \\
	...
	\begin{proposition}
		Let $f, g: X \in [0, \infty]$ be \ul{measurable} functions and $\alpha \geq 0$. Then
		\begin{enumerate}
